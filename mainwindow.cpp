...
MainWindow::MainWindow(QWidget *parent) :
    QMainWindow(parent), ui(new Ui::MainWindow), m_EntryList(),
    m_Database(), m_Model(nullptr)
{
    ui->setupUi(this);
    ...
    if (checkQuery.exec(QString("SELECT path,fileName,hash FROM '%1'").arg(checkTable)))
    {
        ...
        while (checkQuery.next())
        {
                    ...
                    )
            {
                ...
                {
                    report->addMiss(QString("%1/%2 [n/a]\t%3")
                                    .arg(checkQuery.value("path").toString())
                                    .arg(checkQuery.value("fileName").toString())
                                    .arg(checkQuery.value("hash").toString()));
                }
...
{
    QStringList l = QFileDialog::getOpenFileNames(this, tr("Select files..."));
    if (l.length() > 0)
    {
...
{
    QString l = QFileDialog::getExistingDirectory(this, tr("Select directory..."));
    if (!l.isEmpty())
    {
        // update entryList
...
void MainWindow::on_actionAbout_triggered()
{
    ...
    QMessageBox::about(this, tr("About"),
                       QString("<table cellspacing=0 cellpadding=0><tr><td valign=\"top\"><img src=\":/media/FileVerifier.png\" alt=\"\" />"
